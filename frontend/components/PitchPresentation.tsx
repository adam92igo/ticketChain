...
export function PitchPresentation({ concertId, tokenId }: { concertId: string; tokenId: string }) {
  ...
  const slides = [
    ...
    <section className="pitch-slide" key="validation">
      ...
      <div className="pitch-validation-layout"><div className="pitch-validation-score"><strong>17/17</strong><span>lifecycle steps validated on-chain</span><p>Real Sepolia transactions, real gas, publicly visible on Etherscan.</p><small>script: validate-live-deployment.ts</small></div><div className="pitch-validation-points"><p><CheckCircle2 /> Primary purchase (buyTicket)</p><p><CheckCircle2 /> Resale at the capped price</p><p><CheckCircle2 /> Holder proof EIP-191 (signer = on-chain owner)</p><p><CheckCircle2 /> Mark as used at the gate</p><p><CheckCircle2 /> Double-use rejected</p><p><CheckCircle2 /> Cancellation + transfer / resale rejected</p><a href={CONTRACT_ADDRESS ? sepoliaAddressUrl(CONTRACT_ADDRESS) : "#"} target={CONTRACT_ADDRESS ? "_blank" : undefined} rel="noreferrer">Demo contract: 0x89Fb…70DE <ExternalLink size={14} /></a></div></div>
    </section>,
