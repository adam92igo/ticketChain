...
export default function MarketplaceClient({ initialConcertId }: { initialConcertId: string }) {
  const { address, concerts, isSepolia, transactionBusy, getConcertTickets, buyResaleTicket } = useTicketChain();
  const [concertId, setConcertId] = useState(initialConcertId);
  ...
  const selectConcert = useCallback(async (nextConcertId: string) => {
  ...
  useEffect(() => {
    if (initialConcertId) void selectConcert(initialConcertId);
  }, [initialConcertId, selectConcert]);
